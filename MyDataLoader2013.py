...
class MyDataLoader(udata.Dataset):
    def __init__(self, videoFileName, videoFileNameMDN, AudioFileName, type) -> None:
        super().__init__()
        labelPath = "/data1/zhaojunnan/new_FedML/2013/labels"
        if type == "train":
        ...
        for file in os.listdir(videoFileName):
            videoMDN_id = file[:-4]
            #lable_id = file[2:-4]
            lable_id = file.split('_',1)[-1][:-4]
            self.videoList.append(os.path.join(videoFileName, file))
            self.audioList.append(os.path.join(AudioFileName, file))
            
            
            self.videoListMDN.append(os.path.join(videoFileNameMDN, videoMDN_id, videoMDN_id+"_aligned"))
            file_csv = pd.read_csv(os.path.join(labelPath, lable_id+'.csv'))

...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_audios, token_type_ids=None, task_ids=None, position_ids=None):

        seq_length = input_audios.size(1)
        position_ids = torch.arange(
            seq_length, dtype=torch.long, device=input_audios.device
        )
        # print(position_ids)
        # print(position_ids.shape, input_audios.shape)
        position_ids = position_ids.unsqueeze(0).repeat(input_audios.shape[0], 1)
        ...
        embeddings = input_audios + position_embeddings 
        # 没用
        # if self.task_specific_tokens:
...
class BertImageAttention(nn.Module):
    def __init__(self, config):
        ...
        self.self = BertImageSelfAttention(config)   # multiHead
        self.output = BertImageSelfOutput(config)    # norm
...
class BertImageLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention = BertImageAttention(config)  # multiHead Attention
        self.intermediate = BertImageIntermediate(config)  # FeedFoward
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        batch_size, num_words, t_hidden_size = audio_embedding.size()
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(t_start, self.fixed_t_layer):
                with torch.no_grad():
                    audio_embedding, txt_attention_probs = self.layer[idx](
                        audio_embedding, txt_attention_mask
                    )
            ...
            if count == 0 and self.in_batch_pairs: # False
            ...
            if count == 0 and self.FAST_MODE: # False
                audio_embedding = audio_embedding.expand(
                    image_embedding.size(0),
                    audio_embedding.size(1),
                    audio_embedding.size(2),
                )
            ...
            if self.with_coattention: # True
                # do the bi attention.
                image_embedding, audio_embedding, co_attention_probs = self.c_layer[
                    count
                ](
                    image_embedding,
                    image_attention_mask,
            ...
            if output_all_encoded_layers:
                all_encoder_layers_t.append(audio_embedding)
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # self.task_specific_tokens = config.task_specific_tokens  没用

        # initlize the vision embedding 没用
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(input_audio.shape[0], input_audio.shape[1]).type_as(input_audio)
        if token_type_ids is None:
            token_type_ids = torch.zeros(input_audio.shape[0], input_audio.shape[1]).type_as(input_audio)
        if image_attention_mask is None:
            image_attention_mask = torch.ones(
                input_video.size(0), input_video.size(1)
            ).type_as(input_audio)

        # 没用
        # if self.task_specific_tokens:
        #     # extend the mask
        #     mask_tokens = input_audio.new().resize_(input_audio.size(0), 1).fill_(1)
        ...
        if co_attention_mask is None:
            co_attention_mask = torch.zeros(
                input_audio.size(0), input_video.size(1), input_audio.size(1)
        ...
        #embedding 没用
        a_embedding_output = self.embeddings(input_audio) # 带posEmbedding
        v_embedding_output = input_video

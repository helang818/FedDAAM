...
class MyDataLoader(udata.Dataset):
    def __init__(self, videoFileName, videoFileNameMDN, AudioFileName, type) -> None:
        super().__init__()
        labelPath = "/data1/zhaojunnan/new_FedML/2013/DepressionLabels/Labels/DepressionLabels"
        if type == "train":
        ...
        for file in os.listdir(videoFileName):
            ...
            if "Freeform" in file:
                ...
                self.videoListMDN.append(os.path.join(videoFileNameMDN, "Freeform", id, id+"_Freeform_video_aligned"))
                file_csv = pd.read_csv(os.path.join(labelPath, file.replace("_Freeform_video.npy", "_Depression.csv")))
            else:
                self.videoListMDN.append(os.path.join(videoFileNameMDN, "Northwind", id, id+"_Northwind_video_aligned"))
                file_csv = pd.read_csv(os.path.join(labelPath, file.replace("_Northwind_video.npy", "_Depression.csv")))
